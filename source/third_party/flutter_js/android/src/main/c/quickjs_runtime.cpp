...
#define QUICKJS_CHANNEL_SENDNATIVE 2;
    ...
    int js_interrupt_handler(JSRuntime * rt, void * opaque) {
        ...
        if (op != nullptr && op->deadlineNanos > 0 &&
            monotonicNanos() >= op->deadlineNanos) {
            return 1;
        }
        return 0;
    }
    ...
    {
        // QuickJS 2026 removed the non-public JS_IsPromise helper. The public
        // state API returns -1 for non-Promise values and a valid state for
        // Promise objects, which preserves flutter_js's boolean FFI contract.
